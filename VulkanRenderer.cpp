...
const std::string MODEL_PATH = "meshes/Mario.obj";
const std::string TEXTURE_PATH = "textures/mario_main.png";
...
VulkanRenderer::VulkanRenderer() : /// Initialize all the variables
	...
	height(0), width(0), presentQueue(0), graphicsQueue(nullptr), pipelineLayout(0), renderPass(0), swapChain(0), commandBuffers(0), descriptorPool(0), descriptorSets(0), uniformBuffers(0), uniformBuffersMemory(0), swapChainExtent{}, swapChainImageFormat{} {
...
SDL_Window* VulkanRenderer::CreateWindow(std::string name_, int width_, int height_) {
	...
	window = SDL_CreateWindow(name_.c_str(), SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, width_, height_, SDL_WINDOW_VULKAN);
	return window;
}
...
void VulkanRenderer::createInstance() {
	...
	appInfo.pApplicationName = "Game name";
...
void VulkanRenderer::createSurface() {
	if (!SDL_Vulkan_CreateSurface(window, static_cast<VkInstance>(instance), &surface) != VK_SUCCESS) {
...
void VulkanRenderer::createGraphicsPipeline() {
	auto vertShaderCode = readFile("shaders/simpleVert.spv");
	auto fragShaderCode = readFile("shaders/simpleFrag.spv");
